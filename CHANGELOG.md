...
- **Imports over the cap archive instead of truncating**: an import from a backup file that would exceed 200 rows of a type archives the rows it pushes out (the oldest by position, as before) and reports how many rows were written and archived.
