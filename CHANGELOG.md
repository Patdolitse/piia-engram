...
- **Unreviewed rows give way first**: at the cap, staging (unreviewed) rows are moved out before reviewed ones, including staging rows written earlier in the same batch call (bulk writes, note ingestion, session extraction, config and memory reconciliation, imports from other tools). A reviewed row is moved only when no staging row is left to move.
