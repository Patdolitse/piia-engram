...
- **`engram doctor` without `--fix` writes nothing to the memory store.** It opens the store read-only. Its version check still goes online and may write the update-check cache, which now lives in the user cache directory (`ENGRAM_CACHE_DIR` to override; `ENGRAM_NO_UPDATE_CHECK=1` turns the check off) instead of the store.
...
### Fixed
- **Overflow archive at the knowledge cap**: each knowledge type (lessons, decisions) keeps at most 200 active rows. Rows that the cap pushes out of the active file now move to an append-only overflow archive (`knowledge/overflow_archive/`), and the row being written is never the one moved out. The archive is written before the active file, keeps the store's at-rest encryption, never rewrites earlier entries, and is not pruned automatically.
- **Unreviewed rows give way first**: when a write reaches the cap, staging (unreviewed) rows are moved out before reviewed ones, including staging rows written earlier in the same batch call (bulk writes, note ingestion, session extraction, config and memory reconciliation, imports from other tools). A reviewed row is moved only when no staging row is left to move. A later item of the same call that repeats a row the call has just moved out is reported as a duplicate instead of being written again.
- **Overflow is visible**: the write result carries `overflow_archived_ids` whenever rows were archived, batch results list them too, the MCP replies of `add_lesson`, `add_decision` and `memory_store`, the `engram reconcile apply` digest and the startup sync message say how many rows were moved, and each archived row gets an audit entry holding only its id and the reason.
- **Archived rows stay reachable**: `Engram.get_overflow_archived(kind, id)` returns an archived row; `export_all` includes the archive under `overflow_archive`; `backup-plan` lists the archive files. Search, recall and list calls keep returning active rows only, and automatic re-capture and reconcile apply treat active content that is already in the archive as present.
- **Imports over the cap archive instead of truncating**: an import from a backup file that would exceed 200 rows of a type archives the rows it pushes out (the oldest by position, as before), and the import's JSON result reports how many rows were written and archived.
