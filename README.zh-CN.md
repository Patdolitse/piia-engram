...
| | 当前仓库 / 开发事实 |
|---|---|
| 版本口径 | **v4.21.1**（2026-09-26 已核验；最新公开包以 PyPI badge / GitHub Releases 为准）|
...
第一次使用？可以先看更完整的 [首个价值快速上手](docs/quickstart-first-value.zh-CN.md)，按默认 19 个核心工具完成 install -> first memory -> fresh-session recall 这条最短路径；或看完整的 [用户指南](docs/user-guide.zh-CN.md)，覆盖 安装 -> 首个价值 -> 跨工具续接 -> 治理/审批 -> 隐私/数据主权 -> 常见问题。Safe Context、replay、freshness/conflict 和 evidence draft 这类 proposal-only 能力见 [Context governance](docs/context-governance.md)。
...
| 跨工具协作 | ✅ MCP 原生（19 个核心工具）| ❌ 仅 Claude | ❌ 单工具 | ⚠ 需逐工具接入 | ⚠ 需逐工具接入 |
...
官方 `@modelcontextprotocol/server-memory` 存储通用的实体关系知识图谱。piia-engram 专为**开发者身份**设计：它有结构化的用户画像、代码标准、质量要求、经验教训和关键决策字段，加上 59 个知识生命周期管理工具（搜索、审查、合并、跨项目继承）。如果你需要通用实体记忆，用官方 server。如果你希望已配置的 MCP 兼容编程工具从同一份已批准的编码偏好和过往经验开始，用 piia-engram。
...
**存储上限：** 已审核的记忆不会因为存满而被挪走；未审核的记忆在待审队列里至少保留 7 天；被移出的内容进入只追加的溢出归档，可以恢复（`engram retention plan` 查看数量，`engram retention restore <id>` 恢复条目）。
...
piia-engram retention plan   # 只读：各类记忆的数量，以及下一次写入会移入归档的条目
