...
- **Remote telemetry** — Remote sending is a separate opt-in (`engram telemetry remote on`) and requires local telemetry to be enabled first. It sends the same count-only, metadata-only payload to the configured telemetry endpoint (default: `https://engram-telemetry.pp3x325.workers.dev/v1/events`; override with `ENGRAM_TELEMETRY_URL`). It can be disabled with `engram telemetry remote off`.
- **Feedback reports** — Weekly feedback reports are a third, separate opt-in (`engram telemetry feedback on`, or a manually previewed `engram feedback --dry-run` flow). They send count-only anonymous feedback to the configured feedback endpoint (default: `https://engram-telemetry.pp3x325.workers.dev/v1/feedback`; override with `ENGRAM_FEEDBACK_URL`) and are rate-limited.
