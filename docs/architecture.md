...
| [`core.py`](../src/piia_engram/core.py) | ~3942 | `Engram` class facade — `__init__`, schema migration, identity CRUD (profile / preferences / trust_boundaries / quality_standards), knowledge CRUD (add/get/update/archive lessons & decisions), link management, domain & project methods, `export_all` / `import_all` with metadata-only dry-run planning |
