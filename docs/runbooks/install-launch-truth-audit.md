...
only *expected to work* (so we never overclaim integration). Where a claim can
be machine-checked it is backed by a test; where it depends on a third-party GUI
it is labelled as expected-to-work.
...
client config only when you explicitly opt in (`engram setup
--apply-external-config`), and it backs up any file before changing it. See
...
rename or removal fails the audit instead of a user hitting a broken command.
...
| --- | --- | --- |
| Claude Code | per-user MCP config (JSON) | verified (team tested) |
| Claude Desktop | desktop MCP config (JSON) | verified (team tested) |
| Cursor | per-user MCP config (JSON) | verified (team tested) |
| Codex | per-user config (TOML) | verified (team tested) |
| Windsurf / Trae / Cline / others | per-user MCP config (JSON) | community-supported (untested by our team) |

"Verified" means the config shape and launch were exercised by maintainers;
"community-supported" means the path is generated but not routinely tested. This
audit does **not** assert that any GUI renders Engram output a particular way —
only that the launch command and config are produced correctly.
...
It also emits a data-fragmentation warning (via the runtime log, on Engram
...
- It does not claim community-supported clients are regression-tested.
