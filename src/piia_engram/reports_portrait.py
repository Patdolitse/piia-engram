...
class PortraitMixin:
    ...
    def render_user_portrait_html(self, portrait: dict, growth: dict | None = None) -> str:
        ...
        if all_les:
            ...
            label = (f"展开全部 {len(all_les)} 条经验" if zh
                     else f"Expand all {len(all_les)} lessons")
            open_attr = " open" if len(all_les) <= 8 else ""  # short lists open by default
            les_drill = (
                f'<details class="drill"{open_attr}><summary>{esc(label)}</summary>'
        ...
        if all_dec:
            ...
            label = (f"展开全部 {len(all_dec)} 条决策" if zh
                     else f"Expand all {len(all_dec)} decisions")
            open_attr = " open" if len(all_dec) <= 8 else ""  # short lists open by default
            dec_drill = (
                f'<details class="drill"{open_attr}><summary>{esc(label)}</summary>'
        ...
        gen = esc(p.get("generated_at", ""))
        footer = (
            f'由 <span class="brand">ENGRAM</span> 自动生成 · 只读快照，未改动任何记忆 · {gen}'
            if zh else f'Auto-generated by <span class="brand">ENGRAM</span> · read-only snapshot · {gen}'
        )
...
"""
        ...
        return (
            ...
            f'<div class="tagline">{esc("✦ 用户画像 · 全量积累" if zh else "✦ User Portrait · full memory")}</div>'
            '<div class="meta-row">'
