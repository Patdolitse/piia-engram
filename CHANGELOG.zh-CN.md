...
- **批量写入保住本批条目**：同一次批量调用写入的条目（批量写入、笔记导入、会话提炼、配置与记忆对账、从其他工具导入）不会被同一批次后写的条目挤出。
- **溢出可见**：发生归档时，写入结果带有 `overflow_archived_ids`，批量结果同样列出；`add_lesson`、`add_decision`、`memory_store` 的 MCP 回复会说明移走了几条；每条被归档的条目都有一条只含 id 和原因的审计记录。
- **归档条目仍可取回**：`Engram.get_overflow_archived(kind, id)` 返回被归档的条目；`export_all` 在 `overflow_archive` 下包含归档内容；`backup-plan` 会列出归档文件。搜索、召回和列表调用仍只返回活跃条目，自动重新采集会跳过已在归档中的内容。
