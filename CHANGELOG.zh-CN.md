...
- **不带 `--fix` 的 `engram doctor` 对记忆库零写入。** 它以只读方式打开库。版本检查仍会联网，并可能写版本缓存；缓存改放用户缓存目录（可用 `ENGRAM_CACHE_DIR` 指定，`ENGRAM_NO_UPDATE_CHECK=1` 可关闭检查），不再放在库里。
...
### 变更
- **经验和决策的存储上限**（取代 4.20.2 的 200 条上限）：
  - 已审核、在用的记忆不会再因为"满了"被挪走。它们有一个硬上限（每类默认 1000 条）。到上限后，新记忆先进入待审队列，晋升其他记忆会被拒绝并返回 `capacity_full`，不写入任何内容。
  - 未审核的记忆（待审队列）有自己的名额（默认 100 条），并且至少保留 7 天，之后容量规则才可能把它们移入溢出归档。队列到达硬顶（默认 200 条）时，新的未审核记忆直接进入归档，回复里会明确说明（`placement: "archived"`）。
  - 退役的记忆（过时、被拒绝或软归档）在 30 天后移入归档；超过 100 条时会更早移入。
  - 每类 200 条现在是"建议整理"的提示线，不再是硬上限。所有上限都可以用环境变量调整：`ENGRAM_CAP_SOFT`、`ENGRAM_CAP_HARD`、`ENGRAM_REVIEW_QUEUE_MAX`、`ENGRAM_REVIEW_QUEUE_CEILING`、`ENGRAM_REVIEW_MIN_STAY_DAYS`、`ENGRAM_RETIRED_GRACE_DAYS`、`ENGRAM_RETIRED_MAX`。
  - 容量处理不再依赖能否读取版本链关系。正在写入的那一条仍然不会被挪走，本次写入要取代的那条决策也仍然保留。
- **版本历史放在归档里**：编辑一条记忆时，旧版本以 `<id>-prev-v<版本号>` 存进溢出归档，而不是留在活跃文件里。`get_knowledge_history`、决策历史和决策线索都会读取归档，历史保持完整。编辑历史快照仍返回 `snapshot_immutable`；编辑其他已归档条目会返回 `archived`，并提示先恢复。
- **未审核的修订不再遮盖它所修订的记忆**：修订被批准之前，原版照常出现在召回、会话恢复摘要和决策历史中。
- **导入与写入使用同一套规则**：不再按位置截断。超过队列硬顶的待审条目进入归档；如果导入会让已审核记忆超过硬上限，导入会被拒绝且不写入任何内容，除非运行 `engram import <文件> --apply --yes --allow-over-cap`（仅命令行）。覆盖导入会把被替换的本地条目移入归档，并保留本地的关系。备份里的归档部分也会导入；没有 id 的条目在重复导入同一文件时不会重复。导入在一次加锁操作里写完各部分；如果中途中断，再运行一次同样的导入即可完成。导入预览会显示将移入或直接放入归档的条目数。
- `update_knowledge` 的状态只接受 `active`、`outdated` 或 `rejected`。
- 相关经验的挑选会考虑所有可见的经验，而不只是最新的 200 条。

### 新增
- `engram retention plan [--json]`：只读查看已审核、待审、降级、退役各有多少条，归档按原因的计数，以及下一次写入会挪走哪些条目。`engram retention restore <id>` 把已归档的条目恢复回来（仍受硬上限和队列硬顶约束）。
- Dock 回收站也会列出已被移入归档的软归档条目，并可以恢复；版本快照不再出现在回收站里。
- `engram doctor` 和 MCP `doctor` 工具会报告上述计数、时间戳在未来的条目、归档中的残缺行以及中断的导入。健康报告把各类计数和归档并列显示，每周回顾会统计本周移入归档的数量。

### 修复
- 内部记账字段（如 `snapshot_of` 或容量相关的时间戳）不能再通过 MCP 写入内容设置。
- 读取不再等待正在进行的写入；此时跳过访问计数更新。
- 已在归档中的内容不会被再次捕获（未审核写入、MEMORY.md 导入、项目接入），对账只按完全相同的文本匹配归档条目。
- 批量写入中修订了本批刚归档的决策时，会保留修订关系。
- 编辑或合并某条记忆时，这条记忆本身不会被这次写入移入归档；编辑的回复会列出这次写入挪走的其他条目。批量结果会把直接放入归档的条目标为 `archived`。
- 被多次归档的条目读回时是最新状态；`backup-plan` 会统计归档中的残缺行；完整性检查把指向已归档条目的关系视为有效。
- 项目上下文只显示列表形式的技术栈和已知问题；分句时文件名、版本号和网址保持完整。
- `docs/trust.md` 说明了 "verified" 的含义。

## [4.20.2] - 2026-09-23

### 修复
- **知识上限处的溢出归档**：每类知识（lessons、decisions）最多保留 200 条活跃条目。被上限挤出活跃文件的条目现在移入只追加的溢出归档（`knowledge/overflow_archive/`），正在写入的那一条永远不会被挤出。归档先于活跃文件写入，保持记忆库的静态加密形态，从不改写已有记录，也不会被自动清理。
- **未审核条目先让位**：写入达到上限时，先移出 staging（未审核）条目，再动已审核条目；同一次批量调用（批量写入、笔记导入、会话提炼、配置与记忆对账、从其他工具导入）中较早写入的 staging 条目也一样。只有在没有可移出的 staging 条目时，才会移出已审核条目。同一次调用里再次出现刚被移出的相同条目时，按重复处理，不会再写一遍。
- **溢出可见**：发生归档时，写入结果带有 `overflow_archived_ids`，批量结果同样列出；`add_lesson`、`add_decision`、`memory_store` 的 MCP 回复、`engram reconcile apply` 的摘要和启动同步消息会说明移走了几条；每条被归档的条目都有一条只含 id 和原因的审计记录。
- **归档条目仍可取回**：`Engram.get_overflow_archived(kind, id)` 返回被归档的条目；`export_all` 在 `overflow_archive` 下包含归档内容；`backup-plan` 会列出归档文件。搜索、召回和列表调用仍只返回活跃条目，自动重新采集和对账导入会把已在归档中、仍为有效状态的内容视为已存在。
- **导入超出上限时改为归档而非截断**：从备份文件导入会使某类超过 200 条时，被挤出的条目（与以前一样按位置取最旧的）进入归档，导入的 JSON 结果如实报告写入和归档的条数。
- 召回中的 playbook 指针始终不超过 playbook 的预算份额（第一条指针也计入）；`context_preview` 以独立标签显示 playbook 桶。
- agent-transcripts 回退路径只接受末段为 `<uuid>/<same-uuid>.jsonl` 的精确形态；session id 只能由 `[A-Za-z0-9._-]` 组成（形近的 Unicode 字符会回退为时间戳 id）。
- 向量索引的 DDL 只来自固定的语句表，运行时不由数值拼接任何 SQL。
- 无法读取受保护条目集合时，上限处理一律保守跳过；本次写入即将取代的决策（无论自动检测还是显式指定）永远不会被移出。
...
### 安全
- 从公开偏好白名单撤回 `hook_content_digest` 并关闭运行时门。底层实现仍保留但保持休眠，计划在 4.18 重新设计隐私模型；4.17.1 遗留值和手工编辑偏好均无法重新激活。本次变更不宣称物理删除了该功能。

### 修复
- 近期及冷启动 Playbook 召回现在同时要求 `status=active` 与 `tier=verified`，避免 staged Playbook 进入面向模型的启动上下文或占用近期条目预算。
- 项目身份发现现在将 `ELOOP`、`EBADF` 及其它 marker 探测/读取失败视为不可判定，立即回退到精确路径哈希身份，不再继续向上继承祖先仓库。
- 为每个缺口增加独立回归，并在修复前的 `v4.17.1@aec0328` 上验证为红。
...
> 摘要读取器读不出真实 transcript（合成形态解析器）；`hook_content_digest` 偏好
> 无法经公开 API 开启；跨行秘密对可进入 staging。默认仍为关闭，且 4.17.0 的公开
> 偏好 API 无法开启；但手工编辑 `preferences.json` 仍可能激活该路径，因此不能断言
> 不存在已开启部署。另：项目身份派生在不可读目录上不再崩溃（POSIX EACCES），已在
> 4.17.0 窗口内修复。

### 修复（4.17.0 内）
- 项目身份派生在 POSIX 不可读目录上不再抛异常（stat 的 EACCES 传播），由身份矩阵测试在非 root CI runner 上发现。
