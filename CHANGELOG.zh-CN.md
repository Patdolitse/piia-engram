...
- **未审核条目先让位**：达到上限时，先移出 staging（未审核）条目，再动已审核条目；同一次批量调用（批量写入、笔记导入、会话提炼、配置与记忆对账、从其他工具导入）中较早写入的 staging 条目也一样。只有在没有可移出的 staging 条目时，才会移出已审核条目。
...
- **归档条目仍可取回**：`Engram.get_overflow_archived(kind, id)` 返回被归档的条目；`export_all` 在 `overflow_archive` 下包含归档内容；`backup-plan` 会列出归档文件。搜索、召回和列表调用仍只返回活跃条目，自动重新采集和对账导入会把已在归档中的内容视为已存在。
- **导入超出上限时改为归档而非截断**：导入会使某类超过 200 条时，被挤出的条目进入归档，导入结果如实报告写入和归档的条数。
