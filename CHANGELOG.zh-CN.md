...
### 修复
- **知识上限不再静默丢失**：每类知识（lessons、decisions）最多保留 200 条活跃条目。此前向已满的类别写入时，会静默丢掉一条——最旧的一条，或在严格审批开启时丢掉新写入的那条——同时仍报告写入成功。现在被上限挤出的条目会移入只追加的溢出归档（`knowledge/overflow_archive/`），不再删除；正在写入的那一条永远不会被挤出。归档先于活跃文件写入，保持记忆库的静态加密形态，从不改写已有记录，也不会被自动清理。
...
- **导入超出上限时改为归档而非截断**：从备份文件导入会使某类超过 200 条时，被挤出的条目（与以前一样按位置取最旧的）进入归档，导入结果如实报告写入和归档的条数。
...
- 向量索引的 DDL 只来自固定的语句表，运行时不再由数值拼接任何 SQL。
